...
class J5 extends CI_Controller {
	...
	{
		$query = $this->db->query("SELECT nama,comment FROM aux_job_aid WHERE nomor = 'J5';");
		foreach ($query->result() as $row)
		{
			$output['comment'] = $row->comment;
			$header['title'] = "J5 - ".$row->nama;
		}
		...
		$query = $this->db->query("SELECT comment FROM aux_job_aid WHERE nomor = 'J5';");
		foreach ($query->result() as $row)
		{
			$output['comment'] = $row->comment;
		}
		...
		$this->load->view('job_aid/content-j5',$output);
	...
	public function a1()
	{
		...
		$header['title'] = "J5 - A1 Physical Inspection";
		$header['css_files'] = [
		...
		$footer['js_files'] = [
			...
			base_url("assets/mdp/config.js"),
			base_url("assets/mdp/global.js"),
			base_url("assets/job_aid/j5-a1.js"),
		];
		...
		$this->load->view('header',$header);
		$this->load->view('job_aid/content-j5-a1',$output);
		$this->load->view('footer',$footer);		
	}
	...
	public function a1_save(){
		...
		$this->db->query("DELETE FROM `job_aid_j5_a1` where id_pabrik = '$pabrik' AND tanggal = '$tanggal' AND equipment = '$equipment'");
		...
		print_r($datax);
		$this->db->insert_batch('job_aid_j5_a1', $datax);
	}
	...
	public function a1_load(){
		...
		$query = $this->db->query("SELECT * FROM job_aid_j5_a1
	...
	public function a3()
	{
		...
		$header['title'] = "J5 - A3 Infrared Inspection";
		$header['css_files'] = [
			base_url("assets/jexcel/v2.1.0/css/jquery.jexcel.css"),
			base_url("assets/jexcel/v2.1.0/css/jquery.jcalendar.css"),
			base_url("assets/jexcel/v2.1.0/css/jquery.jdropdown.css"),
		...
		$footer['js_files'] = [
			...
			base_url("assets/mdp/config.js"),
			base_url("assets/mdp/global.js"),
			base_url("assets/job_aid/j5-a3.js"),
		];
		...
		$this->load->view('header',$header);
		$this->load->view('job_aid/content-j5-a3',$output);
		$this->load->view('footer',$footer);		
	}
	...
	public function a3_save(){
		...
		$this->db->query("DELETE FROM `job_aid_j5_a3` where id_pabrik = '$pabrik' AND tanggal = '$tanggal' AND equipment = '$equipment'");
		$data_json = $_REQUEST['data_json'];
		$data = json_decode($data_json);
		$datax = array();
		foreach ($data as $key => $value) {
		...
		print_r($datax);
		$this->db->insert_batch('job_aid_j5_a3', $datax);
	}
	...
	public function a3_load(){
		...
		$query = $this->db->query("SELECT * FROM job_aid_j5_a3
		WHERE id_pabrik = '$id_pabrik'
		AND tanggal = '$tanggal'
		AND equipment = '$equipment'
		;");
